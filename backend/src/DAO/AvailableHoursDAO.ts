import { AvailableHoursDTO } from '../DTO/DTO';
import pool from '../database';

export class AvailableHoursDAO {
  async getAll(): Promise<AvailableHoursDTO[]> {
  ...
  async findAvailableHoursByEmployeeId(employeeId: number, reservationDate: string, duration: string): Promise<AvailableHoursDTO[]> {
    const result = await pool.query(`
      ...
      AND ($2 > CURRENT_DATE 
           OR (ah.start_time > (CURRENT_TIME + interval '1 hour') AND $2 = CURRENT_DATE))
